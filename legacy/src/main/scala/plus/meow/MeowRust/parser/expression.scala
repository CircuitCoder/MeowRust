...
trait Expression extends RegexParsers with Literal with Pattern with Identifier with Label with Type {
  override lazy val EXPRESSION: Parser[Expr] = EXPRESSION_WITH_BLOCK | EXPRESSION_WITHOUT_BLOCK
  val STATEMENT: Parser[Stmt]
  ...
  lazy val STATEMENTS: Parser[(List[Stmt], Option[Expr])] = (
      (STATEMENT+(separator?)) ^^ { a => (a, None) }
    | (STATEMENT+(separator?)) ~? EXPRESSION_WITHOUT_BLOCK ^^ { (s, e) => (s, Some(e)) } // Type is last expr
  ...
  // TODO: support qualified path
  lazy val PATH_EXPRESSION: Parser[PathExpr] = (
    PATH_IN_EXPR
  ) ^^ PathExpr
}
